...
static int my_verify_measurements(const char* mrenclave, const char* mrsigner,
                                  const char* isv_prod_id, const char* isv_svn) {
    mbedtls_printf("Entered verify_measurements");
    assert(mrenclave && mrsigner && isv_prod_id && isv_svn);
    mbedtls_printf("No error in assert");
    ...
    if (g_verify_isv_svn &&
        ...
        return -1;
    mbedtls_printf("finished verify_measurements");
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    strcpy(attestation_type_str, "dcap");
...
//EOC
    ...
    while ((ret = mbedtls_ssl_handshake(&ssl)) != 0) {
        printf("Entered the while loop of handshake");
        fflush(stdout);
